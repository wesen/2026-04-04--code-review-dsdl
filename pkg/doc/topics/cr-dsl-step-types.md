...
**Cross-linking (CR-DSL-002):** The file badge and individual line numbers are interactive links. When the walkthrough passes `onOpenFile` to `CRWalkthrough`, clicking the file badge opens the FileViewer at the full line range. Clicking any individual line number opens the FileViewer focused on that single line with the line highlighted.

**URL state:** FileViewer reads `?file=&ref=&lines=&highlight=` query params. The base URL is `/wt/{id}`.
...
**Cross-linking (CR-DSL-002):** The `main → feat/...` ref badges in the diff header are interactive links. Clicking `main` opens the FileViewer in compare mode showing the file at `main`. Clicking `feat/...` opens the FileViewer showing the file at the head ref. Both open in compare mode (two-pane side by side) so the reader can see the before and after simultaneously.

**URL state:** FileViewer reads `?file=&ref=&compare=true&compareRef=&lines=` query params.
...
**Cross-linking (CR-DSL-002):** Each pane header (showing the file, ref, and line range) is an interactive link. Clicking it opens the FileViewer showing that single pane in isolation — useful for seeing the full file around the compared range. The FileViewer shows the same content as the pane but with scroll navigation and line highlighting available.

**URL state:** Same as `source` — `?file=&ref=&lines=&highlight=`.
...
**Cross-linking (CR-DSL-002):** The `src/path/file.ts:42` badge is an interactive link (styled in the accent color, underlined). Clicking it opens the FileViewer at that exact line with the line highlighted. The walkthrough's head ref is used if the annotation has no explicit `ref` field.

**URL state:** Same as `source` — `?file=&ref=&lines=N,N&highlight=N`.
